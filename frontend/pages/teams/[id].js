	...
	if(props.user.is_captain && props.user.team_set[0] == router.query.id){
		...
		for(var k = 0; k < props.teams.users.length; k++){
			...
			if(props.user.id != props.teams.users[k].id){
				console.log(props);
					players.push(<Player key={k} teammate user_id={props.teams.users[k].id} team_id={props.teams.id} name={`${props.teams.users[k].first_name} ${props.teams.users[k].last_name} - ${props.teams.users[k].form}`} ></Player>)
			}
		}
