...
function Teams(props) {
	...
	if(props.user.is_captain && props.user.team_set[0] == router.query.id){
		...
		players.push(<Player captain={props.user.is_captain} user_id={props.user.id} team_id={props.user.team_set[0]} name={`${props.user.first_name} ${props.user.last_name}`} ></Player>)
		
		for(var k = 0; k < props.teams.users.length; k++){
			
			if(props.user.id != props.teams.users[k].id){
					players.push(<Player key={k} teammates={ids} teammate user_id={props.teams.users[k].id} team_id={props.teams.id} name={`${props.teams.users[k].first_name} ${props.teams.users[k].last_name} - ${props.teams.users[k].form}`} ></Player>)
			}
		}
				...
				<Flex backgroundColor="white" p="25px" rounded="lg" flexDirection="column" flexWrap="wrap" margin="50px">
        			...
        			setTimeout(() => {
						...
						{(props) => (
							...
							<Field name="name">
								{({ field, form}) => (
								<FormControl isDisabled flexGrow={1} w={["100%","100%","33%","33%", "33%"]} mr="5px" isRequired isInvalid={form.errors.first_name && form.touched.first_name}>
      					...
      					<Modal isOpen={isOpened} onClose={onClosed}>
      					  ...
      					  <ModalContent>
      					    ...
      					    <ModalFooter>
      					      ...
      					      <Button colorScheme="red" border="0" cursor="pointer" isLoading={props.isSubmitting} onClick={() => {handleDelete(); onClosed(); router.push('/?t=success')}} type="submit">Изтрий</Button>
      					    </ModalFooter>
      					  </ModalContent>
      					</Modal>
	...
	else if(props.user.is_captain == false && props.user.team_set[0] == router.query.id){

		

		players.push(<Player player user_id={props.user.id} team_id={props.user.team_set[0]} name={`${props.user.first_name} ${props.user.last_name}`} ></Player>)
		console.log(props.user.id);
		for(var k = 0; k < props.teams.users.length; k++){
			if(props.user.id != props.teams.users[k].id && props.user.is_captain == props.teams.users[k].is_captain){
				// console.log(props);
				
				players.push(<Player key={k} outside user_id={props.teams.users[k].id} team_id={props.teams.id} name={`${props.teams.users[k].first_name} ${props.teams.users[k].last_name} - ${props.teams.users[k].form}`} ></Player>)
			}
			else if(props.teams.users[k].is_captain){
				players.push(<Player key={k} captain user_id={props.teams.users[k].id} team_id={props.teams.id} name={`${props.teams.users[k].first_name} ${props.teams.users[k].last_name} - ${props.teams.users[k].form}`} ></Player>)
			}
		}
