...
const MenuItems = ({ children }) => (
  <Button
    ...
    _hover={{ backgroundColor: "#85c59b", textDecoration: "none" }}
    background="none"
    _focus={{ outline: "none" }}
    fontFamily="Rubik"
    color="white"
  ...
  if (props.loggedin == 0 && props.inteam == "false") {
    team = (
      <MenuItems display="none">
        <Link display="none" href="/maketeam/">
          <a>Създай отбор</a>
        </Link>
      </MenuItems>
    );
    ...
    team = (
      <MenuItems>
        <Link href={`/teams/${encodeURIComponent(props.inteam)}/`}>
          <a>Моят отбор</a>
        </Link>
      </MenuItems>
    );
  ...
  if (props.loggedin && !isMobile) {
    ...
    login = (
      <>
        <MenuItems>
          <Link href="/login">
            <a onClick={onClose}>Вход</a>
          </Link>
        </MenuItems>
        <MenuItems marginLeft={["none", "none", "none", "auto"]}>
          <Link
...
function ProfileButton(props) {
  return (
    <Button
      marginLeft={["none", "none", "none", "auto"]}
      _active={{ bg: "transparent" }}
      _hover={{ backgroundColor: "#85c59b" }}
      cursor="pointer"
      fontFamily="Rubik"
      color="white"
      bg="transparent"
